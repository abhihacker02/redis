...
class Redis
{
	...
	//hashmap of key-value pair where value stores key-value and time of insertion
	unordered_map<string, pair<string, long>> store;
	//hashmap of key with value as time to live 
	unordered_map<string, long> time_to_live;
	//hashmap of all the ordered sets
	unordered_map<string, set<pair<long, string> >> sorted_sets;
	//hashmap of the {members,score} pair of a particular ordered set
	unordered_map<string, unordered_map<string, long>> member_score_maps;

	//last time when timm_to_clean function was called
	int last_clean_time = time(0);
	...
	//static because this method doesn't belong to a particular object.
	//It behaves same for all the objects
	...
	//method to periodically refresh the ordered set 
	//and erase key-value pair which have expired
	void fixed_time_cleaning()
	{
		//copy of store so that when we erase from original the iterator isn't affected
		unordered_map<string, pair<string, long>> store_copy = store;
		//iterator of copied hashmap
		unordered_map<string, pair<string, long>>::iterator store_iter = store_copy.begin();
		...
		while (store_iter != store_copy.end())
		{
			...
			//time to live of that particular key
			long ttl = time_to_live[key];
	...
	{
		//number of pairs to be added in the set
		int numOfElements = score_member_list.size();
		//to access ordered set of the corresponding key 
		set<pair<long, string>> *cur_sorted_set;
		//to access member-score map of corresponding key
		...
		{
			//each pair of the given list
			pair_elem = score_member_list[i];
			...
			else
			{
				...
				//member which is already present in the set
				string del_member = map_iter->first;
				//previous score which should be updated 
				long del_score = map_iter->second;
	...
	vector<string> ZRANGE(string key, int start_range, int end_range)
	{
		...
		cur_sorted_set = &sorted_sets[key];
		//iterator of current ordered set
		multiset<pair<long, string>>::iterator set_iter = (*cur_sorted_set).begin();
		//size of the set 
		int size_set = (*cur_sorted_set).size();
		//setting valid values if ranges are negative
		setValidRanges(&start_range, &end_range, size_set);
	...
	{
		//vector of desired tuple which we will return
		vector<pair<string, long>> members;
		//ordered set on which we will be working
		set<pair<long, string>> *cur_sorted_set;
		...
		cur_sorted_set = &sorted_sets[key];
		//iterator of current ordered set
		set<pair<long, string>>::iterator set_iter = (*cur_sorted_set).begin();
		//size of the set 
		int size_set = (*cur_sorted_set).size();
		//setting valid values if ranges are negative
		setValidRanges(&start_range, &end_range, size_set);
	...
	int ZRANK(string key, string member)
	{
		...
		//otherwise
		//assigning by pointers
		cur_sorted_set = &sorted_sets[key];
		cur_member_score_map = &member_score_maps[key];
		//iterator on the current ordered set
		set<pair<long, string>>::iterator set_iter = (*cur_sorted_set).begin();
