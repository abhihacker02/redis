...
class Redis{
	...
	private:
		...
		unordered_map<string, pair<string,int> >store; //hashmap of key-value pair where value stores key-value and time of insertion
		unordered_map<string, int>time_to_live;  //hashmap of key with value as time to live 
		unordered_map<string, set<pair<int,string> > >sorted_sets; //hashmap of all the ordered sets
		unordered_map<string, unordered_map<string,int> >member_score_maps; //hashmap of the {members,score} pair of a particular ordered set
	...
	public:
		...
		int writeTimetolive(string fname) {
			unordered_map<string, int> *m=&time_to_live;
        	int count = 0;
        	if (m->empty())
                return 0;
		...
		int writeStore(string fname) {
			unordered_map<string, pair<string,int> > *m=&store;
        	int count = 0;
        	if (m->empty())
                return 0;
        	...
        	for(unordered_map<string, pair<string,int> >::iterator it = m->begin(); it != m->end(); it++) {
        		stringstream ss;
        		ss<<it->second.second;
		...
		int writeSortedSets(string fname) {
			unordered_map<string, set<pair<int,string> > > *m=&sorted_sets;
        	int count = 0;
        	if (m->empty())
                return 0;
        	...
        	for(unordered_map<string, set<pair<int,string> > >::iterator it = m->begin(); it != m->end(); it++) {
        		string key=it->first;
        		for(set<pair<int,string> >::iterator sit=(it->second).begin();sit!=(it->second).end();sit++){
					stringstream ss;
        			string score,member;
        			ss<<(*sit).first;
        			score=ss.str();
		...
		int writeMemberScoreMaps(string fname) {
			unordered_map<string, unordered_map<string,int> > *m=&member_score_maps;
        	int count = 0;
        	if (m->empty())
                return 0;
        	...
        	for(unordered_map<string, unordered_map<string,int> >::iterator it = m->begin(); it != m->end(); it++) {
        		string key=it->first;
        		for(unordered_map<string,int>::iterator mit=(it->second).begin();mit!=(it->second).end();mit++){
					stringstream ss;
        			string score,member;
        			ss<<(*mit).second;
        			score=ss.str();
		...
		int readStore(char* fname){
			...
			while(inFile>>s){
				...
				int insert_time=Redis::toInt(time);
		...
		int readSortedSets(char* fname){
			...
			while(inFile>>s){
				...
				int score=Redis::toInt(scorestr);
		...
		void fixed_time_cleaning(){
			unordered_map<string, pair<string,int> >store_copy= store; //copy of store so that when we erase from original the iterator isn't affected
			unordered_map<string, pair<string,int> >::iterator store_iter=store_copy.begin(); //iterator of copied hashmap
			
			while(store_iter!=store_copy.end()){
				string key=store_iter->first;
				int insert_time=store_iter->second.second;
				int time_now=time(0);
				int time_elapsed=time_now-insert_time;
				int ttl=time_to_live[key];//time to live of that particular key
		...
		string GET(string key){
			try{
				...
				if(store.find(key)!=store.end()){
					int time_now=time(0);
					int insert_time=store[key].second;
					int time_elapsed=time_now-insert_time;
		...
		int EXPIRE(string key,int expiry_time){
		...
		int ZADD(string key,vector<pair<int,string> >score_member_list){
			
			int numOfElements=score_member_list.size();//number of pairs to be added in the set
			set<pair<int,string> >* cur_sorted_set;//to access ordered set of the corresponding key 
			unordered_map<string,int>* cur_member_score_map; //to access member-score map of corresponding key
			
			//if key is not present
			if(sorted_sets.find(key)==sorted_sets.end()){
				//create an ordered set for given key and also create member-score map
				sorted_sets.insert({key,set<pair<int,string> >()});
				member_score_maps.insert({key,unordered_map<string,int>()});
			}
			...
			for(int i=0; i< numOfElements; i++){
				...
				if((*cur_member_score_map).find(member)==(*cur_member_score_map).end()){
					...
					unordered_map<string,int>::iterator map_iter=(*cur_member_score_map).find(member);
					string del_member=map_iter->first; //member which is already present in the set
					int del_score=map_iter->second;    //previous score which should be updated
		...
		vector<string> ZRANGE(string key,int start_range,int end_range){
			...
			set<pair<int,string> >* cur_sorted_set; //ordered set on which we will be working
			...
			cur_sorted_set=&sorted_sets[key];
			multiset<pair<int,string> >::iterator set_iter=(*cur_sorted_set).begin();//iterator of current ordered set
		...
		vector<pair<string,int> > ZRANGE(string key,int start_range,int end_range,string withscore){
			vector<pair<string,int> >members;//vector of desired tuple which we will return
			set<pair<int,string> >* cur_sorted_set; //ordered set on which we will be working
			...
			//assigning set by pointers so that we are accesing the true hashmaps and not the local ones
			cur_sorted_set=&sorted_sets[key];
			set<pair<int,string> >::iterator set_iter=(*cur_sorted_set).begin();//iterator of current ordered set
			int size_set=(*cur_sorted_set).size();//size of the set 
			setValidRanges(&start_range,&end_range,size_set);//setting valid values if ranges are negative
		...
		int ZRANK(string key,string member){
			...
			set<pair<int,string> >* cur_sorted_set;
			unordered_map<string,int>* cur_member_score_map;
			...
			cur_member_score_map=&member_score_maps[key];
			set<pair<int,string> >::iterator set_iter=(*cur_sorted_set).begin();//iterator on the current ordered set
			
			//find score of the member from member-score hashmap so that we can find the pair in the ordered set
			int score=(*cur_member_score_map)[member];
			pair<int,string>item_to_search=make_pair(score,member);
		...
		string ZSCORE(string key,string member){
			...
			unordered_map<string,int>* cur_member_score_map=&member_score_maps[key];
...
int main(){
	...
	while(getline(cin,inp)){
		...
		if(input=="END"){
			...
			vector<pair<int,string> >score_member_list;
			...
			ss>>withscores;
			if(withscores!=""){
				vector<pair<string,int> >members=obj.ZRANGE(key,start_ranges,end_ranges,withscores);
